...
#*** I suspect that this is dealing with the fact that for stateless, memoryless processes, with representables
...
"""
#*** "basis" is the notion of "L_fix"? (See "Lfix limits what Changes Are of Concern in Pattern Matching" in AlgebraicABMs slides)
...
#*** The central "schedule" abstraction is hidden inside Competing Clocks, and is not 
#       directly featured in this code.

#*** ASK: Why are such HomSets associated with a rule?  Do they need to be in the context of naive hom search?
...
#*** ASK: What are the keys here?
Base.haskey(::EmptyHomSet, k) = false

Base.haskey(::RepresentableHomSet, k) = false

#*** Disable the next 4
# Instead, we will have a RuntimeHomSet, which is different than Empty/Representable.  Not necessary to have data
Base.pairs(h::ExplicitHomSet) = pairs(h.val)

Base.getindex(h::ExplicitHomSet, i) = h.val[i]

# KB: We can delete these
...
#  This is declaring the TYPE of the default_sampler
const default_sampler = FirstToFire{
  Union{Pair{Int, Nothing},   # non-explicit homset (Handling Empty pattern & representable pattern)
        Pair{Int, Pair{Int,Int}}, # explicit single connected component homset
        Pair{Int, Vector{Pair{Int,Int}}}},  # explicit multiple connected component homset
  Float64}
...
mutable struct RuntimeABM
  ...
  rt = new(init, init_homset.(abm.rules, Ref(init), Ref(additions(abm))), 
    ...
    #*** i is serving as the index of the rule, to uniquely identify the rule
...
#*** disable, given reference to ExplicitHomSet?
#*** ASK: This seems to require an ExplicitHomSet (including its IncHomSet) for a RegularP
